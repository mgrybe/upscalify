...
class DRCT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # mariusg
            # Pad H and W to multiples of window_size to avoid invalid window partition
            _, _, H_in, W_in = x.shape
            mod_pad_h = (self.window_size - (H_in % self.window_size)) % self.window_size
            mod_pad_w = (self.window_size - (W_in % self.window_size)) % self.window_size
            ...
            # mariusg
            # remove the padding after upsampling (scale = self.upscale)
            if mod_pad_h != 0 or mod_pad_w != 0:
                H_out = (H_in) * self.upscale
                W_out = (W_in) * self.upscale
                x = x[:, :, :H_out, :W_out]
...
if __name__ == "__main__":
    # Build the model (use 'pixelshuffle' because that's what's implemented)
    model = DRCT(
        upscale=4,
        in_chans=3,
        img_size=64,
